...
export class PlayerComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.conqueringScores$ = this.playerService.bindConqueringScores(
      this.playerId
    );
    this.isHovered$ = this.draggingService.bindIsHoveredByCreature(
      this.playerId
    );

    this.subscription.add(
      this.draggable.clickEvent.subscribe(() => {
        this.selectPlayer();
        this.triggerDrop();
      })
    );
    this.subscription.add(
      this.draggable.draggingEvent.subscribe((coordinates) =>
        this.sendDraggingCoordinates(coordinates)
      )
    );
    this.subscription.add(
      this.draggable.dropEvent.subscribe(() => this.triggerDrop())
    );
  }
          ...
          setTimeout(() => {
            if (this.playerElementRef) {
              this.draggingService.registerCoordinates({
                ...
                x: position.pxToPercent(
                  this.playerElementRef.nativeElement.getBoundingClientRect()
                    .left,
                  "x"
                ),
                y: position.pxToPercent(
                  this.playerElementRef.nativeElement.getBoundingClientRect()
                    .top,
                  "y"
                ),
                width: position.pxToPercent(
                  this.playerElementRef.nativeElement.offsetWidth,
                  "x"
                ),
                height: position.pxToPercent(
                  this.playerElementRef.nativeElement.offsetHeight,
                  "y"
                ),
                type: PLAYER_TYPE,
              });
